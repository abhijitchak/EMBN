C---A Fortran code for the maximum likelihood estimation (MLE) for the  Bernoulli ERG model of a bipartite network  
C---Bipartite network: Example bank firm network
C-- Input Parameters....
C--N: No of firms, M: No of banks, L: Total number of links, ni= number
C of parameter to be estimated. ni=1 for bernouli model
C-- nconf: No of ensamble avarage------   
C---iseeds are the intital random number seed....

C---output will be written in "theta_Bernoulli.dat". col1: no of
C-- ensamble avg, col2: no of converge ensamble avg, col3: estimated
C----parameter theta, col4: <theta^2>, col5: std of theta 

      implicit double precision(a-h,o-z)
      parameter (N=10,M=5,L=17,kmax=M,kmax1=N)
      parameter (ni=1,nconf=1000,maxrun1=7+3*ni)
...
C----Expected number of links is the only known observable zo(1)
      zo(1)=L

      write(*,*)dlog(zo(1)/(M*N-zo(1)))
   
C-------Make a copy of the observed network---------------------------------------
      Pneb1=Pneb
      Pdegree1=Pdegree
      Aneb1=Aneb
      Adegree1=Adegree
...
C-----Starting from the observed network generate maxrun1=(7+3*ni) new network using MCMC------ 
     ...
     .iran1,iran2,iran3)
c,Fr2,Fr5,Fr7,Fr9,Br1,Br2,Br3,Br4)
...
C------Optimization: Newton Raphson minimization-----
      ...
      open(1,file='theta_Bernoulli.dat')
      do ir=1,ni
 ...
 1000 enddo
       a=a/2.0d00
       if(isub.gt.35.and.it.eq.6)goto 5000
       if(it.lt.6)goto 18
       if(it.gt.5)goto 17
